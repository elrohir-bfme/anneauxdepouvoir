...
const { data: tags } = await useAsyncData("tags", async () => [
  ...new Set(
    (await queryContent<CustomParsedContent>("blogs/").only(["tags"]).find())
      .map((el) => el.tags)
      .flat()
  ),
]);
...
const { path } = useRoute();
const title = "Articles des Anneaux de Pouvoir";
const description =
  "Articles de la série du 'Le Seigneur des anneaux : Les Anneaux de Pouvoir'";
const img = "https://anneauxdepouvoir.fr/images/articles.jpg";
  ...
  <UContainer class="mt-14 py-3">
    ...
    <div class="lg:flex lg:space-x-8">
      ...
      <div class="w-60 lg:block hidden">
        <BlogsListNavigator :categories="categories || []" :tags="tags || []" />
      </div>
    </div>
  </UContainer>
</template>
